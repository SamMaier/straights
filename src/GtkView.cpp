...
GtkView::GtkView(Game* game, GameController* controller): game_(game), controller_(controller), handBox_(true, 10){
...
void GtkView::setHandImages() {
    for (int card = 0; card < gameState_.hand.size(); card++) {
        cardsInHand[card]->set(images_.getCardImage(gameState_.hand[card]));
    }
}
